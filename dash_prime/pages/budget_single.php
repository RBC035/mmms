<?php
	...
	// to get ministerID
	$selmin=mysqli_query($con,"select * from minister where LeaderID ='".$leaderow['LeaderID']."'");
    ...
    // event name
	$no = 1;
	$eventquery = mysqli_query($con,"select * from event where EventID='".$EID."'");
	while ($fetchev=mysqli_fetch_array($eventquery)) 
		{
			...
			$query=mysqli_query($con,"select event .* , requirement .* from event , requirement where event.EventID = requirement.EventID && MinisterID='".$minrow['MinisterID']."' && EventName='".$fetchev['EventName']."'");
