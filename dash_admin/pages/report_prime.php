<?php
	...
	if ($getID == 0) 
	{
		...
		$fpdf -> Cell(284,8,'PRIME MINISTERS REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
		...
		$seleader =mysqli_query($con,"select student .* , selectedleader .* from student , selectedleader where student.RegNo = selectedleader.RegNo && StudentStatus='Enable'");
			while ($leaderow=mysqli_fetch_array($seleader)) 
			  {
				$selemini=mysqli_query($con,"select * from minister where LeaderID = '".$leaderow['LeaderID']."' && MinisterPossition='Prime minister'");
	...
	else 
	{
		// third line
		$fpdf -> SetTextColor(0,0,0);
		$fpdf -> Cell(284,8,'FORMER PRIME MINISTERS REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
