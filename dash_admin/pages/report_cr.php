<?php
	...
	if ($getID == 0) 
	{
		...
		$fpdf -> Cell(284,8,'CLASS REPRESENTATIVE REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
		...
		$fpdf -> Cell(29,8,'SelectedYear',1,0);
		$fpdf -> Cell(20,8,'Level',1,0);
		$fpdf -> Cell(23,8,'Gender',1,0);
		$fpdf -> Cell(35,8,'CrStatus',1,0);
		$fpdf -> Cell(48,8,'ProgramName',1,0);
		...
		while($row=mysqli_fetch_array($query))
			{
				...
				$fpdf -> Cell(29,8,$row['SelectedYear'],1,0);
				$fpdf -> Cell(20,8,$row['Level'],1,0);
				$fpdf -> Cell(23,8,$row['Gender'],1,0);
				$fpdf -> Cell(35,8,$row['CrStatus'],1,0);

				//to find program
				$sel=mysqli_query($con,"select * from program where RegNo = '".$row['RegNo']."' ");
				while ($prow=mysqli_fetch_array($sel)) 
				{
					$fpdf -> Cell(48,8,$prow['ProgramName'],1,0);
				}
				//to find department
	...
	else 
	{
		// third line
		$fpdf -> SetTextColor(0,0,0);
		$fpdf -> Cell(284,8,'FORMER CLASS REPRESENTATIVE REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
		...
		$fpdf -> Cell(26,8,'SelectedYear',1,0);
		$fpdf -> Cell(20,8,'Level',1,0);
		$fpdf -> Cell(46,8,'ProgramName',1,0);
		// $fpdf -> Cell(30,8,'Department',1,0);
		$fpdf -> Cell(29,8,'Start date',1,0);
		$fpdf -> Cell(29,8,'End date',1,0);
		$fpdf -> Cell(45,8,'Duration',1,0);
		$fpdf -> Cell(23,8,'Description',1,0);

		// code to fecth
		$query=mysqli_query($con,"select student .* , cr .* from student , cr where student.RegNo = cr.RegNo && StudentStatus='Enable' && CrStatus != 'CR' &&  CrStatus != 'Assistance'");
		if (mysqli_num_rows($query)>0) 
		{
			while ($row= mysqli_fetch_array($query)) 
			{
				...
				$fpdf -> Cell(26,8,$row['SelectedYear'],1,0);
				$fpdf -> Cell(20,8,$row['Level'],1,0);
				...
				$programSelect = mysqli_query($con,"select * from program where RegNo = '".$row['RegNo']."'");
					while ($programQuery = mysqli_fetch_array($programSelect)) 
						{
							$fpdf -> Cell(46,8,$programQuery['ProgramName'],1,0);
						}
				...
				$DurationSelect = mysqli_query($con,"select * from duration where Reason != 'Continue' && LeaderID ='".$row['RegNo']."'");
				while ($DurationQuery = mysqli_fetch_array($DurationSelect)) 
					{
						...
						$fpdf -> Cell(29,8,$startDate,1,0);
						$fpdf -> Cell(29,8,$endDate,1,0);
						$fpdf -> Cell(45,8,$monthDay ,1,0);
		...
		else
		{
			...
			$fpdf -> SetFont('Times','',11);
			$fpdf -> SetTextColor(12,22,102);
			$fpdf -> Cell(284,8,'There is not data available in this table while will be released will apear here.................',1,0);
		}
	}
