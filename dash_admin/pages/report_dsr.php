<?php
	...
	if ($getID == 0) 
	{
		...
		$fpdf -> Cell(284,8,'DEPARTMENT STUDENT REPRESENTATIVE REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
		...
		$fpdf -> Cell(25,8,'Status',1,0);
		$fpdf -> Cell(20,8,'Gender',1,0);
		$fpdf -> Cell(62,8,'ProgramName',1,0);
		$fpdf -> Cell(32,8,'Department',1,0);


		// code to fecth
		$query=mysqli_query($con,"select student .* , selectedleader .* from student , selectedleader where student.RegNo = selectedleader.RegNo && StudentStatus='Enable' && Possition != 'Former DSR' && Possition != 'Former President' && Possition != 'Former Vice President'  && Possition != 'President' && Possition != 'Vice President'");
		while($row=mysqli_fetch_array($query))
			{
				...
				$fpdf -> Cell(25,8,$row['Possition'],1,0);
				...
				{
					$fpdf -> Cell(62,8,$prow['ProgramName'],1,0);
				}
				//to find department
	...
	else 
	{
		// third line
		$fpdf -> SetTextColor(0,0,0);
		$fpdf -> Cell(284,8,'FORMER DEPARTMENT STUDENT REPRESENTATIVE REPORT IN MASO GOVERNMENT',0,0,'C');
		$fpdf -> Ln(10);
		...
		//code to excute std leader
		$query=mysqli_query($con,"select student .* , selectedleader .* from student , selectedleader where student.RegNo = selectedleader.RegNo && StudentStatus='Enable' && Possition != 'President'  && Possition != 'Vice President' && Possition != 'Former President'  && Possition != 'Former Vice President'  && Possition != 'DSR' ");
		while($row=mysqli_fetch_array($query))
			{
				$fpdf -> Ln(8);
				$fpdf -> SetFont('Times','',11);
				...
				$programSelect = mysqli_query($con,"select * from program where RegNo = '".$row['RegNo']."'");
					while ($programQuery = mysqli_fetch_array($programSelect)) 
						{
							$fpdf -> Cell(60,8,$programQuery['ProgramName'],1,0);
						}
